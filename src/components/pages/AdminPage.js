...
const AdminPage = () => {
  ...
  // Use useCallback to ensure the fetchMovies function is not recreated on every render
  ...
  }, [page, searchQuery, activeSort, API_URL]);

  // Run fetchMovies on page, searchQuery, or activeSort change
  useEffect(() => {
    fetchMovies();
  }, [fetchMovies]);
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
              ...
              <Grid item xs={12} sm={6} md={4} lg={3} key={movie._id}>
                <Paper elevation={3} sx={{ p: 2 }}>
                  ...
                  <Box display="flex" justifyContent="space-between" sx={{ mt: 2 }}>
                    <IconButton onClick={() => handleMovieModalClick(movie)} color="primary">
                      <InfoIcon />
                    </IconButton>
                    <IconButton color="primary" onClick={() => openUpdateDialog(movie)}>
                      <Edit />
                    </IconButton>
                    <IconButton color="error" onClick={() => openDeleteDialog(movie)}>
                      <Delete />
                    </IconButton>
                  </Box>
                </Paper>
      ...
      <AddMovie open={addDialogOpen} onClose={closeAddDialog} onMovieAdded={fetchMovies} />
      <UpdateMovie open={updateDialogOpen} onClose={closeUpdateDialog} movie={selectedMovie} onMovieUpdated={fetchMovies} />
      <DeleteMovieDialog open={deleteDialogOpen} onClose={closeDeleteDialog} onDelete={handleDeleteMovie} movieName={selectedMovie?.name} />
